    ...
    <div>
      ...
      {!loading && data?.totalElements === 0 ? (
        <div style={{
          ...
          <p style={{ fontSize: "1.1rem", fontWeight: 600 }}>Nenhum anime encontrado</p>
          <p style={{ fontSize: "0.875rem", marginTop: "0.5rem" }}>Tente outros filtros</p>
        </div>
      ) : (
        <div style={{
          display: "grid",
      ...
      {data && data.totalPages > 1 && (
        <div style={{
          ...
          <PaginationButton page={page - 1} disabled={page === 0} label="← Anterior" searchParams={searchParams} />

          {Array.from({ length: data.totalPages }, (_, i) => i)
            .filter((i) => i === 0 || i === data.totalPages - 1 || Math.abs(i - page) <= 2)
            .map((i, idx, arr) => (
              <span key={i}>
                {idx > 0 && arr[idx - 1] !== i - 1 && (
                  <span style={{ color: "var(--text-muted)", padding: "0 0.25rem" }}>…</span>
                )}
                <PaginationButton page={i} current={i === page} label={String(i + 1)} searchParams={searchParams} />
              </span>
            ))
          }

          <PaginationButton page={page + 1} disabled={data.last} label="Próxima →" searchParams={searchParams} />
        </div>
      )}
      ...
      {data && (
        <p style={{ textAlign: "center", marginTop: "1.5rem", fontSize: "0.8rem", color: "var(--text-muted)" }}>
          {data.totalElements} anime{data.totalElements !== 1 ? "s" : ""} encontrado{data.totalElements !== 1 ? "s" : ""}
        </p>
      )}
    </div>
  );
...
function PaginationButton({
  ...
  return (
    <a
      href={disabled ? undefined : `/animes?${params.toString()}`}
      style={{
        ...
        color: current ? "var(--background)" : disabled ? "var(--text-muted)" : "var(--text-primary)",
